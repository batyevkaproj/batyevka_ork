...
import {
  ...
  } from "@/components/ui/carouselOrange";

import { type CarouselApi } from "@/components/ui/carouselOrange";
...
import slide1 from '@/public/img/homeSlider/img_landing.svg';
import slide2 from '@/public/img/homeSlider/internet.svg'

const SliderHome = ({ theme, business }: HeaderProps) => {
...
<Carousel className={`min-[2430px]:mt-[60px] mt-[45px]`} setApi={setApi}>
  ...
  <CarouselContent className='flex'>
    ...
    <CarouselItem className='flex items-center justify-center max-[910px]:flex-col-reverse'>
        <div className='min-[2430px]:ml-[170px] ml-[120px] max-[1706px]:w-[655px] max-[1400px]:w-[430px] z-10 max-[910px]:ml-0 max-[910px]:w-fit max-[910px]:px-[35px]'>
            ...
            <div className='max-w-[555px]'>
                ...
                <h2>і телебачення</h2>
                <h4>в Солом'янському районі Києва</h4>
  ...
  <div className={'max-[2429px]:w-[55px] max-[2429px]:h-[258px] w-[72px] h-[321px] absolute top-[84px] min-[2430px]:top-[78px] right-0 max-[910px]:hidden'}>
        <CarouselPreviousVariation className='bg-white text-[#dc622d] max-[2429px]:size-[40px] size-[52px] -translate-y-0 rotate-90 border-0 relative top-0 left-0'/>
        <div className='max-[2429px]:mr-[23px] mr-[29px] max-[2429px]:pt-[10px] pt-[13px]'>  
            {timesArray.map((_, index) => (
            <span className='' key={index}>
                <span className='text-right text-[#5f6061] text-[26px] leading-[36px] max-[2429px]:text-[20px] max-[2429px]:leading-[32px]'>{index+1==current ? <><p key={index} className='text-[#dc662d] pb-[13px] max-[2429px]:pb-[10px]'>0{index+1}</p><span className='h-[3px] max-[2429px]:h-[2px] bg-[#dc622d] block w-1/2 relative right-[-49px] max-[2429px]:right-[-38px] top-[-33px] max-[2429px]:top-[-27px]'></span></>:<p key={index} className='text-[#5f6061] pb-[13px] max-[2429px]:pb-[10px]'>0{index+1}</p> }</span>
            </span>
            ))}  
        </div>
        <CarouselNextVariation className='bg-white text-[#dc622d] max-[2429px]:size-[40px] size-[52px] rotate rotate-90 -translate-y-0 border-0 relative top-0 left-0'/>
    </div>
</Carousel>
    </>
     );
