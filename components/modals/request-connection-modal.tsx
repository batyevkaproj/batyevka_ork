...
export const RequestConnectionModal = () => {
  ...
  const phoneRef = useRef<HTMLInputElement | null>(null); // ✅ канонічний тип
  ...
  const onSubmit = async (formData: RequestConnectionFormData) => {
    ...
    try {
      ...
      const orderData = {
        ...
        comment: `Заявка на БІЗНЕС підключення. Адреса: ${selectedAddress.street.name}, ${selectedAddress.house.number}`,
      };
  ...
  return (
    <Dialog open={isModalOpen} onOpenChange={onClose}>
      <DialogContent className="shadow-[0_4px_29px_0px_#0B273C] bg-[#133853] rounded-[10px] max-w-[620px] p-0 border-0">
        <DialogHeader>
          ...
          <DialogDescription className="flex flex-col items-center text-[16px] mt-[15px] font-light text-white">
            <p>
              {data?.speed} {data?.measure} – {data?.price} грн/міс
            </p>
            {data?.hasStaticIp && (
        ...
        <form
          ...
          className="flex mt-[30px] gap-[30px] flex-col items-center mb-[60px] w-full"
        >
          <div className="h-[60px] flex items-center w-3/4">
            <input
              ...
              {...register("name", {
                ...
                maxLength: { value: 50, message: "Ім'я занадто довге" },
              })}
            />
          </div>

          {errors.name && (
            <span className="text-red-500 text-sm mt-[-20px]">
              {errors.name.message}
            </span>
          )}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-3/4 rounded-full bg-[#2F94AD] h-[60px] text-[18px] font-semibold text-[#133853] shadow-[0_4px_20px_0_#56AABF80] disabled:opacity-50"
          >
